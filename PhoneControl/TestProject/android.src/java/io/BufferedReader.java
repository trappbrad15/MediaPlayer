/*    */ package java.io;
/*    */ 
/*    */ public class BufferedReader extends Reader
/*    */ {
/*    */   public BufferedReader(Reader in)
/*    */   {
/*  5 */     throw new RuntimeException("Stub!"); } 
/*  6 */   public BufferedReader(Reader in, int size) { throw new RuntimeException("Stub!"); } 
/*  7 */   public void close() throws IOException { throw new RuntimeException("Stub!"); } 
/*  8 */   public void mark(int markLimit) throws IOException { throw new RuntimeException("Stub!"); } 
/*  9 */   public boolean markSupported() { throw new RuntimeException("Stub!"); } 
/* 10 */   public int read() throws IOException { throw new RuntimeException("Stub!"); } 
/* 11 */   public int read(char[] buffer, int offset, int length) throws IOException { throw new RuntimeException("Stub!"); } 
/* 12 */   public String readLine() throws IOException { throw new RuntimeException("Stub!"); } 
/* 13 */   public boolean ready() throws IOException { throw new RuntimeException("Stub!"); } 
/* 14 */   public void reset() throws IOException { throw new RuntimeException("Stub!"); } 
/* 15 */   public long skip(long charCount) throws IOException { throw new RuntimeException("Stub!"); }
/*    */ 
/*    */ }

/* Location:           C:\Users\Brad\Downloads\adt-bundle-windows-x86_64-20130717\sdk\platforms\android-18\android.jar
 * Qualified Name:     java.io.BufferedReader
 * JD-Core Version:    0.6.2
 */